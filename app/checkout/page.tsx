...
export default function CheckoutPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-6 font-sans grid grid-cols-1 lg:grid-cols-3 gap-12 text-slate-900">
      <div className="lg:col-span-2 space-y-12">
        ...
        <div className="space-y-6">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.4em] flex items-center gap-3"><MapPin size={18} className="text-orange-500" /> 01. Địa chỉ nhận hàng (TP.HCM)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
              <Building2 className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
              <select value={selectedDistrict} onChange={(e) => setSelectedDistrict(e.target.value)} className="w-full h-16 pl-14 pr-6 bg-white border-2 border-slate-100 rounded-3xl font-black outline-none focus:border-orange-500 appearance-none shadow-sm cursor-pointer">
                {HCMC_DISTRICTS.map(d => <option key={d.id} value={d.id}>{d.label} - Ship: {d.fee.toLocaleString()}đ</option>)}
              </select>
            </div>
            <input value={detailedAddress} onChange={(e) => setDetailedAddress(e.target.value)} placeholder="Số nhà, tên đường..." className="w-full h-16 px-8 bg-white border-2 border-slate-100 rounded-3xl font-black outline-none focus:border-orange-500 shadow-sm" />
          </div>
        </div>

        <div className="space-y-6">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.4em]">02. Chi tiết tiền từng người ({owners.length})</h3>
          <div className="grid grid-cols-1 gap-4">
            {owners.map(name => (
              <div key={name} className={`p-6 bg-white border-2 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-6 transition-all ${payingMember === name ? 'border-orange-500 ring-4 ring-orange-500/10' : 'border-slate-100 hover:border-slate-200 shadow-sm'}`}>
                <div className="flex items-center gap-5">
                  <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 font-black text-2xl uppercase italic shadow-inner">{name[0]}</div>
                  <div className="flex flex-col">
                    <p className="font-black text-slate-900 uppercase italic text-xl tracking-tight">{name === "Host" ? `Bạn (${user?.name})` : name}</p>
                    <div className="flex flex-col gap-0.5 mt-1">
                      <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Tiền món (sau giảm): <span className="text-slate-900 ml-1">{(calculateUserSubtotal(name) * (1 - discountPercent/100)).toLocaleString()}đ</span></p>
                      <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Ship chia đều: <span className="text-orange-500 ml-1">+{Math.round(shipPerPerson).toLocaleString()}đ</span></p>
                    </div>
                  </div>
                </div>
                <button onClick={() => setPayingMember(name)} className="flex items-center gap-3 bg-slate-900 text-white px-8 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest hover:bg-orange-500 transition-all shadow-lg active:scale-95">
      ...
      <div className="relative">
        <div className="bg-[#111827] text-white p-10 rounded-[3rem] sticky top-28 shadow-2xl min-h-[500px] flex flex-col items-center justify-between text-center border border-white/5 overflow-hidden">
          {payingMember ? (
