    ...
    return (
      <div className="min-h-[85vh] flex items-center justify-center px-6 bg-slate-50/30">
        <motion.div 
          ...
          className="max-w-md w-full bg-white rounded-[3.5rem] p-12 text-center shadow-2xl shadow-slate-200/50 border border-slate-100 space-y-8"
        >
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 font-sans bg-slate-50/30 min-h-screen relative">
        ...
        {showConfirmDelete && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md bg-white rounded-[3.5rem] shadow-2xl border border-slate-100 p-10 text-center"
            >
              ...
              <p className="text-slate-400 text-sm font-bold uppercase tracking-wide px-4">
                Mọi kỷ niệm ăn uống của bạn sẽ biến mất vĩnh viễn khỏi trình duyệt này.
              </p>
              <div className="grid grid-cols-2 gap-4 mt-10">
                <button 
                  onClick={() => setShowConfirmDelete(false)} 
                  className="py-5 bg-slate-100 text-slate-500 rounded-[2rem] text-[11px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all"
                >
                  Giữ lại
                </button>
                <button 
                  onClick={handleClearHistory} 
                  className="py-5 bg-red-500 text-white rounded-[2rem] text-[11px] font-black uppercase tracking-widest hover:bg-red-600 shadow-xl shadow-red-200 transition-all"
                >
                  Xóa sạch
                </button>
              </div>
              <button onClick={() => setShowConfirmDelete(false)} className="absolute top-8 right-8 text-slate-300 hover:text-slate-900"><X size={24} /></button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
      ...
      <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        {orders.length > 0 && (
          <motion.button 
            ...
            className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-red-500 transition-all bg-white px-8 py-4 rounded-2xl shadow-sm border border-slate-100"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-28 bg-slate-900 rounded-[3.5rem] p-1 shadow-2xl">
            <div className="bg-white rounded-[3.3rem] p-10 text-center relative overflow-hidden">
              <div className="absolute -top-10 -right-10 w-32 h-32 bg-orange-500/10 rounded-full blur-3xl" />
              
              <motion.div whileHover={{ rotate: 10 }} className="w-32 h-32 bg-slate-900 rounded-[2.8rem] flex items-center justify-center text-white mx-auto mb-8 shadow-2xl relative">
              ...
              <div className="grid grid-cols-1 gap-3 mt-10">
                <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 flex items-center justify-between px-8 transition-colors hover:bg-orange-50">
                ...
                <div className="bg-slate-900 p-6 rounded-[2rem] flex items-center justify-between px-8 text-white transition-all hover:bg-orange-500 group">
                  <div className="text-left">
          ...
          <AnimatePresence mode="popLayout">
            {orders.length > 0 ? (
              orders.map((order, idx) => (
                ...
                >
                  {/* ORDER HEADER */}
                  <div className="bg-slate-50/80 px-8 py-5 flex items-center justify-between border-b border-slate-100 group-hover:bg-orange-50/30 transition-colors">
                    <div className="flex items-center gap-6">
                      <span className="bg-slate-900 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase italic shadow-lg shadow-slate-200">
                        #{order.id}
                      </span>
                  ...
                  <div className="p-8 md:p-10 space-y-8">
                    {order.items.map((item, i) => (
                      <div key={i} className="flex items-center justify-between group/item p-2 rounded-3xl hover:bg-slate-50 transition-colors">
                        <div className="flex items-center gap-6">
                          <div className="relative w-20 h-20 rounded-[2rem] overflow-hidden border-4 border-white shadow-xl group-hover/item:scale-105 transition-transform duration-500">
                            <Image 
                          ...
                          <div>
                            <p className="text-lg font-black text-slate-900 uppercase italic leading-tight group-hover/item:text-orange-500 transition-colors tracking-tight">
                              {item.name}
                            </p>
                    ...
                    <div className="pt-8 border-t-2 border-dashed border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-8">
                      ...
                      <Link 
                        href="/products" 
                        className="w-full sm:w-auto px-10 py-5 bg-slate-900 text-white rounded-[1.8rem] text-[11px] font-black uppercase tracking-widest hover:bg-orange-500 hover:shadow-[0_20px_40px_-10px_rgba(249,115,22,0.4)] transition-all flex items-center justify-center gap-3 active:scale-95 group/btn"
                      >
              ...
              >
                ...
                <h4 className="text-2xl font-black text-slate-900 uppercase italic tracking-tighter">Bạn chưa có kỷ niệm ăn uống nào</h4>
                <p className="text-slate-400 font-medium mt-2 max-w-xs mx-auto text-sm">Hãy bắt đầu đặt món và lấp đầy lịch sử của mình nhé!</p>
                <Link 
                  href="/products" 
                  className="inline-block mt-8 bg-orange-500 text-white px-10 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl shadow-orange-100"
                >
                  Bắt đầu đặt món
                </Link>
              </motion.div>
